...
%vincoli di ordine.
propedeuticita(fondamentiDiIctEParadigmiDiProgrammazione, progettazioneDiBaseiDiDati).
propedeuticita(tecnicheeStrumentiDiMarketingDigitale, introduzioneAlSocialMediaManagement).
propedeuticita(comunicazionePubblicitariaEComunicazionePubblica, laGestioneDelleRisorseUmane).
propedeuticita(tecnologieServerSidePerIlWeb, progettazioneESviluppoDiApplicazioniWebSuDispositiviMobileI).
...
% vincolo 7
% per ogni Materia2 di ordineSemestre(Materia1,Materia2) tutte le lezione(....) devono avere settimana,giorno,ora > rispetto a materia1
...
% vincoli auspicabili: 
% vincolo 1: la distanza tra la prima e l’ultima lezione di ciascun insegnamento non deve superare le 6 settimane
:- lezione(Materia, _, Settimana1, _, _), lezione(Materia, _, Settimana2, _, _),
   |Settimana1 - Settimana2| > 6.
...
:- lezione(crossMediaArticolazioneDelleScrittureMultimediali, _, Settimana, _, _), Settimana < 16.
:- { lezione(crossMediaArticolazioneDelleScrittureMultimediali, _, Settimana, _, _) : Settimana = 16} < 1.
:- lezione(introduzioneAlSocialMediaManagement, _, Settimana, _, _), Settimana < 16.
:- { lezione(introduzioneAlSocialMediaManagement, _, Settimana, _, _) : Settimana = 16} < 1.

% vincolo 4:
% la distanza tra l'ultima lezione di mobileI e la prima di mobileII non deve essere > 2 settimane
:- UltimaLezione = #max{ Settimana : lezione(progettazioneESviluppoDiApplicazioniWebSuDispositiviMobileI, _, Settimana, _, _) },
   PrimaLezione = #min{ Settimana : lezione(progettazioneESviluppoDiApplicazioniWebSuDispositiviMobileII, _, Settimana, _, _) },
   PrimaLezione - UltimaLezione > 2.

% vincolo 3
% Materia2 deve iniziare dopo almeno 4 ore di Materia1
:-  propedeuticita(Materia1, Materia2), lezione(Materia2, Settimana, Giorno, Ora), { lezione(Materia1, Settimana1, Giorno1, Ora1) : Settimana1 * 100 + Giorno1 * 10 + Ora1 <= Settimana * 100 + Giorno * 10 + Ora } < 4.
